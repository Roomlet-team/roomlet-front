...
interface WorkspaceWithdrawApi {
  WorkspaceId: number;
}

const deleteWorkspaceWithdrawApi = async (data: WorkspaceWithdrawApi): Promise<WorkspaceInfo> => {
  const response = await clientInstance.delete(`/v1/workspace/@${data.WorkspaceId}/mypage/me`);

  return response.data;
};

/**
 * React-query를 사용하여 워크스페이스를 탈퇴하는 커스텀 훅
 * @returns
 * - 다음 객체를 반환합니다:
 *   - `data`: 워크페이스를 탈퇴하기 위한 데이터
 *   - `error`: 에러 발생 시 에러 객체
 ...
 */
const useDeleteWorkspaceMypageMeQuery = () => {
...
export default useDeleteWorkspaceMypageMeQuery;
